package com.binder.helpdesk.api.controller;
...
import com.binder.helpdesk.api.dto.Summary;
...
import com.binder.helpdesk.api.response.Response;
import com.binder.helpdesk.api.security.jwt.JwtTokenUtil;
import com.binder.helpdesk.api.service.TicketService;
import com.binder.helpdesk.api.service.UserService;
...
public class TicketController {
	...
	@PostMapping()
	@PreAuthorize("hasAnyRole('CUSTOMER')") // quem pode criar um ticket somente cliente.
	...
	public ResponseEntity<Response<Ticket>> findById(@PathVariable("id") String id) {
		...
		// Retorna a lista de alterações daquele ticket pesquisado
		Iterable<ChangeStatus> changesCurrent = ticketService.listChangeStatus(ticket.getId());

		// Com a lista pesquisada , carrega a list aqui
		for (Iterator<ChangeStatus> iterator = changesCurrent.iterator(); iterator.hasNext();) {

			ChangeStatus changeStatus = iterator.next();
			changeStatus.setTicket(null);
			changes.add(changeStatus); // add em uma list chamada changes.
		}

		// add list changes na list
		// add no objeto ticket um list(change). Pois dentro da Entity Ticket tem um ->
	...
	public ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, @PathVariable int page,
		...
		// se o perfil do usuario logado for igual a técnico - lista todos os tickets ,
		// independente do cliente
		if (userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
			tickets = ticketService.listTicket(page, count);

			// se o perfil do usuario logado for igual a cliente lista somente os tickets do
			// usuario logado
	...
	// Metodo que altera o status do ticket - ex: tecnico pode aceitar ticket,
	// cilente pode reprovar,
	// tecnico pode fechar ticket, cliente pode fechar ticket
	...
	public ResponseEntity<Response<Ticket>> changeStatus(@PathVariable("id") String id,
		...
		try {
			...
			// se for assinado - pega o usuario logado
			...
			// inclue no diretorio a alteração de status do ticket
			ChangeStatus changeStatus = new ChangeStatus();

			// usuario que fez a alteração (usuario que está logado)
			changeStatus.setUserchanged(userFromRequest(request));
			// data de alteração
			changeStatus.setDateChangedStatus(new Date());
			// status do ticket
			changeStatus.setStatus(StatusEnum.getStatus(status));
			// ticket que foi alterado
			changeStatus.setTicket(ticketPersisted);

			// cria as alteraçõe e salva o ticket alterado.
	...
	// Metodo que vai dar um resumo - Aprovados, reprovados, atribuidos , fechados
